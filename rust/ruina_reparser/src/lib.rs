...
#[inline(always)]
pub fn get_key_page_locales_by_id(id: &str) -> HashMap<Locale, &KeyPageLocale> {
    get_locales_by_identifier(&KEY_PAGE_LOCALES, id)
}
...
mod tests {
    ...
    fn abno_page_locales_map_sanity_check() {
        ABNO_PAGES.iter().for_each(|abno_page| {
    ...
    fn battle_symbol_locales_map_sanity_check() {
        BATTLE_SYMBOLS.iter().for_each(|battle_symbol| {
    ...
    fn combat_page_locales_map_sanity_check() {
        ...
        COMBAT_PAGES.iter().for_each(|combat_page| {
    ...
    fn key_page_locales_map_sanity_check() {
        ...
        KEY_PAGES.iter().for_each(|key_page| {
            let locales = get_key_page_locales_by_id(key_page.id);
            let locale_count = locales.values().len();
